...
Global repo guidance lives in this file. Path-scoped Python rules are in `.agents/rules/`:
...
- `agent-rules-layout.md` — how to add/remove rules and maintain tool symlinks

Claude Code and Cursor read the same content via symlinks in `.claude/rules/*.md` and `.cursor/rules/*.mdc`. Canonical rule files live in `.agents/rules/`; see `agent-rules-layout.md` when creating or deleting rules. Python rules load when working on files under `src/` or `tests/`.
