...
class MulticastRangeInfoEntry(BaseModel, validate_assignment=True):
    ...
    def add_range(self, start_ip: IPvAnyAddress, end_ip: IPvAnyAddress) -> "MulticastRangeInfoEntry":
...
class MulticastPools(BaseModel):
    ...
    @property
    def pool_list(self) -> List[MulticastRangeInfoEntry]:
        return self._pool_list

    def get_pool_by_name(self, name: str) -> MulticastRangeInfoEntry:
        for pool in self.pool_list:
            if pool.id == name:
                return pool
        raise ValueError(f"Pool with label '{name}' not found in the VideoIPath System Preferences.")
