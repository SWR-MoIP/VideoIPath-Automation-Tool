...
class SuperProfile(BaseModel):
    ...
    maxBand: float | int = Field(
        default=20, ge=0, description="Maximum IP bandwidth (Mbit/s)"
    )  # TODO Was jetzt? Int oder Float?
    ...
    # - Transport Stream
    tsband: float | int = Field(
        default=10, ge=0, description="TS bandwidth (Mbit/s)"
    )  # TODO Was jetzt? Int oder Float?
    ...
    ipband: int = Field(default=15, ge=0, description="IP bandwidth (Mbit/s)")  # New GUI-Profile Default: 15
