...
class TopologyAPI(BaseModel):
    ...
    # Deprecated
    def _get_next_free_slot(self, base_switch_device_id: str):
        # https://bu-vip.swr.ard/rest/v2/data/config/network/nGraphElements/* where maps.0.x=600 and maps.0.y>0/**

        device = self._fetch_nGraphElement_by_key(base_switch_device_id)  # type: BaseDevice
        label = device.factory_label
        if "LEAF" in label:
            mode = "LEAF"
        elif "SPINE" in label:
            mode = "SPINE"
        else:
            raise ValueError("Device is not a switch device")
        if mode == "LEAF":
            x_val = device.maps[0].x
            y_val = device.maps[0].y
            if y_val >= 0:
                operator = ">"
            else:
                operator = "<"
            data = self.vip_connector.http_get_v2(
                f"/rest/v2/data/config/network/nGraphElements/* where type='baseDevice' and maps.0.x={x_val+200} and maps.0.y{operator}0/**"
            )
            ...
            used_positions = []
            if data.data["config"]["network"]["nGraphElements"]["_items"]:
                for item in data.data["config"]["network"]["nGraphElements"]["_items"]:
                    y = item["maps"][0]["y"]
                    used_positions.append(y)

            while pos_y in used_positions:
                if operator == ">":
                    pos_y += 200
                else:
                    pos_y -= 200
            ...
            y_val = device.maps[0].y + 400
            data = self.vip_connector.http_get_v2(
                f"/rest/v2/data/config/network/nGraphElements/* where type='baseDevice' and maps.0.y={y_val} and maps.0.x>0/**"
            )
            ...
            while pos_x in used_positions:
                pos_x += 250

            return pos_x, pos_y

    # Get device configuration from the topology / from the driver

    # def sync_device(self, device: TopologyDevice):
    #     """ Update a device configuration in the topology.
    ...
    #     body = self._generate_nGraphElement_action(update_list=[device.configuration.base_device] + device.configuration.codec_vertices + device.configuration.generic_vertices + device.configuration.ip_vertices + device.configuration.internal_edges + device.configuration.external_edges, add_list=[], remove_list=[])
    #     return self.vip_connector.http_patch_v2("/rest/v2/data/config/network/nGraphElements", body)
